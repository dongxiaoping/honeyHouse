...
Page({
  ...
  updatePayModel: function () {
    ...
    dataAccess.getUserInfoByWechatId({
      data: {
        id: app.globalData.userInfo.wechat_id
      },
      callback: function (status, res2) {
        ...
        if (res2.status === globalConst.interfaceStatus.SUCCESS) {
          ...
          let isUserAccountPay = false;
          if (accountCash - allGoodsPrice >= 0) {
            isUserAccountPay = true;
          }
          that.setData({
            isUserAccountPay: isUserAccountPay
          });
        }
      }
    });
